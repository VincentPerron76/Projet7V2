...
def test_home(client):
    ...
    assert "Bienvenue sur l'API de prediction" in response.data.decode("utf-8")  # Décoder en UTF-8 pour les caractères spéciaux

# Test de la route /bonjour
def test_bonjour(client):
    response = client.get("/bonjour?SK_ID_CURR=234285")
    assert response.status_code == 200
    assert "vous avez demande l'identifiant 234285" in response.data.decode("utf-8")  # Décoder en UTF-8 pour les caractères spéciaux
...
def test_predict_valid(client):
    response = client.get("/predict?SK_ID_CURR=180213")
    assert response.status_code == 200
    response_json = json.loads(response.data)
    assert "SK_ID_CURR" in response_json
    assert "prediction" in response_json
